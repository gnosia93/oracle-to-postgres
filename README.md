...
* 통상적으로 onprem의 오라클 데이터베이스를 AWS로 이전시 마이그레이션을 위한 네트워크는 VPN(1.2Gbps/s) 또는 DX(Max 40Gbps/s) 를 사용하게 되는데, 허용 가능한 서비스 다운타임에 따라 네트워크의 종류와 bandwidth 를 선택하게 됩니다. 네트워크 선택시 초기 데이터 로딩의 데이터 볼륨과 변경 데이터 볼륨(ongoing 데이터 적재량)을 고려하여 설계하여야 합니다. 
