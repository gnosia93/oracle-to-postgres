...
```
ec2 인스턴스들에 ssh 로 로그인 하기 위해서는 Key paris 가 필요합니다. 터미널에서 ssh-keygen 명령어를 이용하여 홈디렉토리 밑에 .ssh 경로에 tf_key 라는 이름의 키페어를 만들도록 합니다. 테라폼 스크립트에서 키페어의 경로를 static 하게 참조하고 있으므로, 생성되는 키페어의 디렉토리 경로와 명칭은 변경해서는 안됩니다.
