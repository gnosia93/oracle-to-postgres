...
리플리케이션 인스턴스는 AWS 클라우드 상에 생성되는 복제 전용 서버로 원본 데이터베이스의 트랜잭션 량에 따라서 인스턴스의 크기와 EBS 볼륨의 크기를 설정하게 됩니다(기본값은 50GB)
엔드포인트의 경우 소스 및 타켓 데이터베이스에 접속하기 위한 설정으로 오라클의 경우 로그 마이너 방식과 바이너리 리더 방식을 지원하고 있습니다. 
본 워크샵에서는 오라클용 엔드 포인트를 로그 마이너용 1개와 바이너리 리더용 1개를 각각 만들게 됩니다. 
엔드 포인트의 설정 항목중 extra_connection_attributes 의 값이 로그 마이너 방식인 경우 설정하지 않으며, 바이너리 모드인 경우에는 "useLogminerReader=N; useBfile=Y" 로 설정하게 됩니다. 
