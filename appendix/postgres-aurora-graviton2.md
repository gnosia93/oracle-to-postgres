...
성능 테스트시 적용되는 Aurora PostgreSQL 데이터베이스의 파리미터 값은 EC2 PostgreSQL 에서 적용한 값과 동일한 값을 적용하는데, wal log 관련 파라미터는 Aurora PostgreSQL의 분산 스토리지 구조상 지원하지 않는 파라미터 이므로 제외합니다. Aurora 의 경우 파라미터 그룹을 먼저 만든 후, 변경을 필요한 값을 설정하게 되는데, shared buffers 의 경우 총 메모리 사이즈가 아니라 블록수로 입력해야 합니다. 
```
[postgresql.conf]
